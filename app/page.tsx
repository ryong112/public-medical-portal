...
import { 
  ...
  ChevronLeft, ChevronRight, X, Clock, CalendarDays, Lock, Archive
} from 'lucide-react';
...
export default function IntegratedPortal() {
  ...
  // 채팅 드래그 함수 (에러 해결 핵심)
  const onMouseDownChat = (e: React.MouseEvent) => {
  ...
  const onDeleteCategoryWithFiles = async (id: number, catName: string) => {
    ...
    const confirmMsg = targetFiles.length > 0 
      ? `'${catName}' 분류를 삭제하시겠습니까?\n내부에 저장된 파일 ${targetFiles.length}개도 서버에서 완전히 삭제되어 용량이 확보됩니다.`
      : `'${catName}' 분류를 삭제하시겠습니까?`;
  ...
  const handleDownloadCategoryZip = async () => {
    ...
    try {
      const promises = targetFiles.map(async (file) => {
        const res = await fetch(file.url); const blob = await res.blob(); zip.file(file.name, blob);
      });
      await Promise.all(promises);
      const content = await zip.generateAsync({ type: "blob" });
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(content);
      link.download = `${selectedCategory}_백업_${new Date().toLocaleDateString()}.zip`;
      link.click();
  ...
  const getFileIcon = (fileName: string) => {
    ...
    if (ext === 'jpg' || ext === 'png' || ext === 'gif') return { icon: <ImageIcon size={iconSize} className="text-purple-500" />, color: 'bg-purple-50', label: 'IMG' };
    if (ext === 'zip' || ext === '7z') return { icon: <FileBox size={iconSize} className="text-yellow-600" />, color: 'bg-yellow-50', label: 'ZIP' };
      ...
      <main className="flex-1 flex overflow-hidden">
        <aside className="w-80 bg-[#EBEEF2] border-r border-slate-300 flex flex-col shadow-[inset_-4px_0_12px_rgba(0,0,0,0.03)] h-full">
          ...
          <div className="p-6 border-t border-slate-300 bg-[#EBEEF2]">
            <div className="flex gap-2 bg-white p-2.5 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-blue-100 shadow-sm transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
              ...
              <button onClick={async() => { if(newCatName.trim()) { await supabase.from('categories').insert([{ name: newCatName.trim(), order_index: categories.length }]); setNewCatName(''); } }} className="bg-slate-900 text-white w-8 h-8 rounded-xl shadow-sm font-black text-sm">+</button>
            </div>
          </div>
        </aside>
          ...
          {isDragOver && (
            <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none animate-in fade-in duration-200">
              <div className="bg-blue-600/90 text-white px-10 py-6 rounded-[40px] shadow-2xl font-black flex flex-col items-center gap-4 animate-bounce">
                <FilePlus size={48} />
                <p className="text-xl">파일을 이곳에 놓으세요!</p>
              </div>
            </div>
          )}
                ...
                <div className="group flex items-center gap-4 mb-2">
                  {isEditingTitle && viewMode === 'files' ? (
                    <div className="flex items-center gap-3 w-full">
                      <input className="text-4xl font-black text-slate-900 tracking-tighter bg-white border-b-4 border-blue-600 outline-none w-full max-w-2xl py-1" value={editTitleValue} onChange={(e)=>setEditTitleValue(e.target.value)} autoFocus onKeyDown={(e) => e.key === 'Enter' && onUpdateCategoryName()} />
                    ...
                    <>
                      ...
                      {viewMode === 'files' && selectedCategory !== '전체' && <button onClick={() => { setEditTitleValue(selectedCategory); setIsEditingTitle(true); }} className="opacity-0 group-hover:opacity-100 bg-slate-100 text-slate-400 p-2 rounded-xl hover:text-blue-500 text-xs font-bold transition-all">✎ 수정</button>}
                      {viewMode === 'files' && <button onClick={handleDownloadCategoryZip} disabled={isDownloadingAll} className="flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-black hover:bg-blue-600 hover:text-white transition-all ml-2 shadow-sm"><Archive size={14} /><span>{isDownloadingAll ? '압축 중...' : '전체 다운로드(ZIP)'}</span></button>}
                    </>
                  )}
                </div>
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {files.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()) && (selectedCategory === '전체' || f.category === selectedCategory)).map(file => {
                        ...
                        return (
                          <tr key={file.id} className="group hover:bg-slate-50/50 transition-all hover:translate-x-1">
                            <td className="py-6 px-4">
                              <div onClick={() => handleDownload(file.url, file.name)} className="flex items-center gap-4 cursor-pointer">
                                <div className={`w-12 h-12 ${info.color} rounded-2xl flex flex-col items-center justify-center border border-transparent group-hover:border-slate-200 shadow-sm transition-all`}>
                                  {info.icon}
                                  <span className="text-[7px] font-black text-slate-400 mt-0.5">{info.label}</span>
                                </div>
                                <span className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{file.name}</span>
                              </div>
                            </td>
